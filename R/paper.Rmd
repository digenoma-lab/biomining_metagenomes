---
title: "Systemix Article: Combined MAG-SMAG Profiling"
author: "Moises A. Rojas"
date: "2023-12-01 // mod. Mar 2024"
...
# Save attributes
smag_md1_sel <- smag_md1 %>%
  dplyr::select("mag_name", "mag_num", "Ecosystem", "Continent")
...
## Section 1: In-depth taxonomy analysis 

### Biomining MAGs vs SMAG taxa comparison
...
mags_metadata_bac <- mags_metadata %>%
  dplyr::filter(kingdom == "Bacteria" & !is.na(ecosystem)) #n=32975
...
plot_depth_top_phyla <- ggplot(mags_md_top_phy, aes(x = freq, y = reorder(phy_top, freq))) +
  geom_bar(stat = "identity", fill = "#459682") +
  geom_text(aes(label = freq), vjust = 0.5, hjust = -0.5, size = 2.5) +
...
plot_depth_spec <- ggplot(mags_depth_spec) +
  ...
  coord_flip()
ggsave("plot_md_biom_species.pdf", plot_depth_spec, height = 2.5, width = 8)
...
plot_depth_usgbs <- ggplot(mags_depth_usgbs_eco,
                           aes(x = num, y = reorder(phylum, num), fill = ecosystem)) +
  geom_col() +
...
plot_depth_kggbs_biom <- ggplot(mags_depth_kggbs_biom,
  ...
  geom_col(position = position_stack(reverse = TRUE)) +
  scale_x_continuous(limits = c(0, 101), n.breaks = 5) + #check!
  scale_fill_manual(values = mags_colors_eco) +
...
mags_qc <- read.csv("mags-all/2-statistics/checkm_parsed_bins.tsv", sep = "\t")
...
# Plot
plot_mags_qc <- ggplot(mags_qcs, aes(x = mags, y = fct_reorder(label, mags), fill = label)) +
  geom_col(position = "dodge2") +
  scale_fill_manual(values = c("#DB7971", "#5CA650", "#859AC7")) +
  labs(x = "Number of MAGs", y = "") +
  theme_classic() +
  theme(legend.position = "none")
ggsave("mags-all/2-statistics/plots/plot_biom_MAGs_QC.pdf", plot_mags_qc, height = 3, width = 5)
```
...
rm(tree_tips, tree_com, tree_con, tree_st, tree_gs, tree_gc, tree_pg) #clean memory
...
#ggsave("fig_1.png", fig_1, height = 14, width = 14)
...
#fig_s1[[2]][[2]] <- fig_s1[[2]][[2]] + theme(axis.text.y = element_text(vjust = 0))
#fig_s1 <- wrap_plots(plot_depth_usgbs, plot_depth_spec, nrow = 2, ncol = 1, heights = c(0.85, 0.15))
ggsave("mags-all/fig_s1.pdf", fig_s1, height = 9, width = 18)
#ggsave("mags-all/fig_s1.png", fig_s1, height = 9, width = 18)
```
...
anova_boxplot_cu <- ggboxplot(kegg_bubble_cu, x = "ecosystem", y = "count_norm",
  ...
  geom_hline(yintercept = mean(kegg_bubble_cu$count_norm), linetype = 2, linewidth = 0.2) +
  labs(x = "Ecosystem", y = "KOs/Mb for copper") +
  rotate_x_text(angle = 45) + #ylim(c(0, 0.25)) +
  #stat_summary(fun = mean, geom = "point", shape = 18, size = 2.5, color = "#FC4E07") +
  stat_compare_means(method = "anova", label.y = 0.25) + #add global annova p-value
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = ".all.", hide.ns = TRUE)
ggsave("plot_annot_boxplot_copper.pdf", anova_boxplot_cu, height = 7, width = 7)
...
anova_boxplot_fe <- ggboxplot(kegg_bubble_fe, x = "ecosystem", y = "count_norm",
  ...
  geom_hline(yintercept = mean(kegg_bubble_fe$count_norm), linetype = 2, linewidth = 0.2) +
  labs(x = "Ecosystem", y = "KOs/Mb for iron") +
  rotate_x_text(angle = 45) +
  stat_compare_means(method = "anova", label.y = 0.4) + #add global annova p-value
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = ".all.", hide.ns = TRUE, label.y = 0.35)
ggsave("plot_annot_boxplot_iron.pdf", anova_boxplot_fe, height = 7, width = 7)
...
anova_boxplot_s <- ggboxplot(kegg_bubble_s, x = "ecosystem", y = "count_norm",
  ...
  geom_hline(yintercept = mean(kegg_bubble_s$count_norm), linetype = 2, linewidth = 0.2) +
  labs(x = "Ecosystem", y = "KOs/Mb for sulfur") +
  rotate_x_text(angle = 45) +
  stat_compare_means(method = "anova", label.y = 0.7) + #add global annova p-value
  stat_compare_means(label = "p.signif", method = "t.test",
                     ref.group = ".all.", hide.ns = TRUE)
ggsave("plot_annot_boxplot_sulfur.pdf", anova_boxplot_s, height = 7, width = 7)
...
kegg_bubbles_plot <- kegg_bubble %>%
  ...
  labs(title = "", x = "Ecosystem", y = "Highlighted KO definitions") +
...
ggsave("plot_annot_bubbles.pdf", kegg_bubbles_plot, height = 7, width = 8.5)
...
```{r fig2}
# Merge plots using patchwork
fig_2 <- { { anova_boxplot_cu / anova_boxplot_fe +
    plot_layout(ncol = 1, widths = c(0.75)) } | anova_boxplot_s / kegg_bubbles_plot } +
  plot_annotation(tag_levels = list(c("a", "b", "c", "d"))) &
  theme(plot.tag =  element_text(face = "bold"))
#adjust y-label margin for sulfur plot
fig_2[[2]][[1]] <- fig_2[[2]][[1]] + theme(axis.title.y = element_text(vjust = -127))
ggsave("mags-all/fig_2.pdf", fig_2, height = 14, width = 18)
#ggsave("mags-all/fig_2.png", fig_2, height = 14, width = 18) #vjust=120
```

## Section 3: SNP Variant Calling
...
#buildref(cdsfile = "mags-all/5-SNPs/dndscv/buildref/cds_node_25854.txt",
#         genomefile = "mags-all/5-SNPs/dndscv/buildref/node_25854.fa",
#         outfile = "mags-all/5-SNPs/dndscv/buildref/testref.rda")
#load("mags-all/5-SNPs/dndscv/buildref/testref.rda")

# Buildref for all bins (very slow! ~2 hrs in building object)
#buildref(cdsfile = "mags-all/5-SNPs/dndscv/buildref/cds_table.txt",
#         genomefile = "mags-all/5-SNPs/dndscv/buildref/S15_bins.fa",
#         outfile = "mags-all/5-SNPs/dndscv/buildref/S15_refcds.rda")

# Inspect refdb
#load("mags-all/5-SNPs/dndscv/buildref/S15_refcds.rda")
#print(RefCDS[[1]])
#rm(gr_genes, RefCDS) #clean memory

# Get annotations (parsed from the VCF generated with SnpEff)
#sampleID chr pos ref mut
snp_mutations <- read.csv("mags-all/5-SNPs/dndscv/run/S15_mutations.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

# Run dndscv (runtime ~8 min, high memory usage) and save object
#snp_dndsout <- dndscv(snp_mutations, refdb = "mags-all/5-SNPs/dndscv/buildref/S15_refcds.rda",
#                      max_muts_per_gene_per_sample = Inf, max_coding_muts_per_sample = Inf,
#                      cv = NULL, outmats = TRUE)
#save(snp_dndsout, file = "mags-all/5-SNPs/dndscv/run/S15_dndsout.rda")
#save(substmodel, file = "mags-all/5-SNPs/dndscv/run/S15_substmodel.rda")
#load("mags-all/5-SNPs/dndscv/run/S15_dndsout.rda")
#load("mags-all/5-SNPs/dndscv/run/S15_substmodel.rda")
...
snp_plot_dndsglob <- ggplot(snp_global_dnds, aes(x = genes, y = reorder(label, genes), fill = label)) +
  ...
  theme_classic() +
  theme(legend.position = "none")
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_global.pdf", snp_plot_dndsglob, height = 2, width = 5)
...
snp_refcds <- read.csv("mags-all/5-SNPs/dndscv/buildref/cds_table.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

# Load contig to bin matrix
snp_bins <- read.csv("mags-all/5-SNPs/dndscv/results/S15_DASTool_contig2bin.tsv", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

# Load gene names (parsed from genes.gff file)
snp_genids <- read.csv("mags-all/5-SNPs/dndscv/results/gene_id_name.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
...
if (file.exists("mags-all/5-SNPs/dndscv/results/ps_genes.txt") == FALSE) {
  write.table(snp_psgenes_sub %>% dplyr::select(gene_name) %>% distinct(gene_name), "mags-all/5-SNPs/dndscv/results/ps_genes.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
}
if (file.exists("mags-all/5-SNPs/dndscv/results/kofam/ps_genes_ids.txt") == FALSE) {
  write.table(unique(snp_psgenes_sub$name), "mags-all/5-SNPs/dndscv/results/kofam/ps_genes_ids.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
}

# Plot global driver genes by MAG/species
snp_plot_psbins <- ggplot(snp_dnds, aes(x = num, y = reorder(bin, num), fill = bin)) +
  ...
  facet_grid(.~species)
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_psbins.pdf", snp_plot_psbins, height = 9, width = 10)

# Add completeness & contamination barplots, then merge
snp_plot_psbins2 <- ggplot(snp_dnds, aes(x = completeness, y = reorder(bin, num), fill = bin)) +
  geom_col(fill = "#3c4d76") +
  geom_vline(xintercept = 100, linetype = 1, linewidth = 0.2) +
  labs(x = "Completeness", y = "") +
  theme_classic() +
  theme(axis.text.y = element_blank(),
        axis.line.y = element_blank(),
        axis.ticks.y = element_blank())
snp_plot_psbins3 <- ggplot(snp_dnds, aes(x = contamination, y = reorder(bin, num), fill = bin)) +
  geom_col(fill = "#763c4d") +
  geom_vline(xintercept = max(snp_dnds$contamination), linetype = 1, linewidth = 0.2) +
...
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_psbins.pdf", snp_plot_psbins, height = 9, width = 12)
rm(snp_plot_psbins2, snp_plot_psbins3) #clean memory

# Lollipop plot of PS genes
snp_plot_psgenes <- ggplot(snp_psgenes_sub,
                           aes(x = wmis_cv, y = reorder(name, wmis_cv)), size = wmis_cv) +
  geom_segment(aes(x = 0, xend = wmis_cv, y = name), color = "gray") +
  geom_point(alpha = 0.7, color = "#E31A1C") +
  geom_text(aes(label = signif, x = -0.15), size = 3, color = "#3c4d76") +
  geom_text(aes(label = path, x = -0.15), size = 2, color = "#3c4d76") +
  geom_vline(xintercept = 1, linetype = 1, linewidth = 0.2) +
  geom_vline(xintercept = max(snp_psgenes_sub$wmis_cv), linetype = 1, linewidth = 0.2) +
  labs(x = "PS strength", y = "Gene CDS") +
  theme_classic()
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_psgenes.pdf", snp_plot_psgenes, height = 11, width = 7)

# Plot PS genes mutations per type, then merge
snp_plot_muts <- ggplot(snp_psgenes_sub, aes(x = n_mut, y = reorder(name, wmis_cv), fill = mut)) +
  geom_col(position = position_stack(reverse = TRUE)) +
  ...
  theme(legend.position = c(0.85, 0.15),
        ...
        axis.line.y = element_blank(),
        axis.ticks.y = element_blank())
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_psmuts.pdf", snp_plot_muts, height = 11, width = 7)

snp_plot_psgenes <- (snp_plot_psgenes | snp_plot_muts) +
  plot_layout(nrow = 1, ncol = 2, widths = c(0.55, 0.5))
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_psgenmuts.pdf", snp_plot_psgenes, height = 11, width = 11)
rm(snp_plot_muts) #clean memory

# Load negative selected (NS) genes from selected pathways
snp_nsgenes <- read.csv("mags-all/5-SNPs/dndscv/results/selected_cds_metals.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
...
if (file.exists("mags-all/5-SNPs/dndscv/results/kofam/ns_genes_ids.txt") == FALSE) {
  write.table(snp_nsgenes$gene_id, "mags-all/5-SNPs/dndscv/results/kofam/ns_genes_ids.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
}
...
snp_plot_nsgenes <- ggviolin(snp_nsgenes, x = "pathway", y = "wmis_cv", color = "pathway",
                             ...
                             order = snp_sort_ns$pathway, add = "jitter", legend = "none",
                             palette = snp_sort_ns$color) +
  scale_y_continuous(limits = c(-0.2, 1.3)) +
  geom_hline(yintercept = 1, linetype = 1, linewidth = 0.2) +
  geom_hline(yintercept = mean(snp_nsgenes$wmis_cv), linetype = 2, linewidth = 0.2) +
  stat_compare_means(method = "anova", label.y = 1.3) +
  stat_compare_means(comparisons = list(c("Copper", "Iron"), c("Sulfur", "Nickel"),
                                        c("Copper", "Nickel")), label = "p.format", method = "t.test")
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_nsgenes.pdf", snp_plot_nsgenes, height = 9, width = 7)
...
snp_plot_nsgensub <- ggdotchart(snp_nsgenes_sub, x = "product", y = "dnds", color = "pathway",
                                xlab = "NS gene product", ylab = "Mean dN/dS", y.text.col = TRUE,
                                rotate = TRUE, palette = snp_nsgenes_sub_col, dot.size = 3,
                                sorting = "descending", ggtheme = theme_pubr()) +
  scale_y_continuous(limits = c(0, 1)) +
  geom_hline(yintercept = mean(snp_nsgenes$wmis_cv), linetype = 2, linewidth = 0.2) +
  geom_hline(yintercept = 1, linetype = 1, linewidth = 0.2)
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_nsgenesprod.pdf", snp_plot_nsgensub, height = 9, width = 10)
...
# Sort phylum by median and set color
snp_estim_sort_phy <- snp_estim_phy %>%
  dplyr::group_by(phylum) %>%
  dplyr::summarise(n = median(dnds_glob)) %>% #by mean?
  dplyr::arrange(desc(n))
snp_estim_sort_phy$color <- rep("#4c6194", nrow(snp_estim_sort_phy))
...
snp_plot_estim_phy <- ggboxplot(snp_estim_phy, x = "phylum", y = "dnds_glob", color = "phylum",
  ...
  geom_hline(yintercept = mean(snp_estim_phy$dnds_glob), linetype = 2, linewidth = 0.2) +
  geom_hline(yintercept = 1, linetype = 1, linewidth = 0.2) +
...
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_estim_phy.pdf", snp_plot_estim_phy, height = 9, width = 7)
...
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_estim_bin.pdf", snp_plot_estim_bin, height = 9, width = 8)

# Load lists of PS and NS genes (externally* matched against KEGG KOs from KOfamScan hits)
snp_kops <- read.csv("mags-all/5-SNPs/dndscv/results/kofam/ps_genes_kos.txt", sep = "\t", header = FALSE, stringsAsFactors = FALSE)
snp_kons <- read.csv("mags-all/5-SNPs/dndscv/results/kofam/ns_genes_kos.txt", sep = "\t", header = FALSE, stringsAsFactors = FALSE)
...
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_kos.pdf", snp_plot_kos, height = 9, width = 12)
...
if (file.exists("mags-all/5-SNPs/dndscv/results/circos/S15.49_genes.txt") == FALSE) {
  write.table(snp_cirmag$gene_name, "mags-all/5-SNPs/dndscv/results/circos/S15.49_genes.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
}
if (file.exists("mags-all/5-SNPs/dndscv/results/circos/contigs/circos_S15.49.txt") == FALSE) {
  write.table(snp_cirmag$contig, "mags-all/5-SNPs/dndscv/results/circos/contigs/circos_S15.49.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
}

# Read parsed gene annotation (externally* processed)
snp_cirgenes <- read.csv("mags-all/5-SNPs/dndscv/results/circos/S15.49_genes_pos.txt", sep = "\t", stringsAsFactors = FALSE)
...
snp_cirgenpos <- read.csv("mags-all/5-SNPs/dndscv/results/circos/contigs/circos_S15.49_relpos.txt", sep = "\t", stringsAsFactors = FALSE)
...
# Circos plot
circos.clear()
pdf("mags-all/5-SNPs/dndscv/results/plot/snp_dnds_circos.pdf", height = 4, width = 4)
...
circos.track(snp_cirmagname, ylim = snp_cirsize, panel.fun = function(x, y) {
  ...
  text(0, 0, snp_cirmagname, cex = 0.65)
  text(0, -0.14, paste("Genome size 2703949 bp", "phylum Actinobacteriota | genus Acidithrix",
                       "168 contigs | N50 26686 | GC 47.7", sep = "\n"), cex = 0.55)
  circos.axis(h = "top", major.at = snp_cirran, labels = paste0(round(snp_cirran/10^6, 1)), #, " Mb"
              labels.cex = 0.35, lwd = 0.7, labels.facing = "clockwise")
}, bg.col = "#6a9b85", bg.border = FALSE, track.height = 0.06)
circos.genomicLabels(snp_cirgenes, labels.column = 7, cex = 0.35, line_lwd = 0.5,
                     side = "inside", line_col = "black", col = "black",
                     connection_height = 0.05, labels_height = 0.06)
dev.off()

### Enrichment ###
## PS genes for enrichment (we need KEGG KOs from KOfamScan)
#snp_psenrich <- read.csv("mags-all/5-SNPs/dndscv/results/kofam/ps_genes_kos.txt", sep = "\t", header = #FALSE, stringsAsFactors = FALSE)
#
...
#
## Dotplot
#pdf("mags-all/5-SNPs/dndscv/results/plot/snp_dnds_enrichdot.pdf", height = 9, width = 7)
#dotplot(snp_keggenrich, showCategory = 30)
#dev.off()
#
## Barplot
#pdf("mags-all/5-SNPs/dndscv/results/plot/snp_dnds_enrichbar.pdf", height = 9, width = 7)
#barplot(snp_keggenrich, showCategory = 30)
#dev.off()
#
## Category netplot
#pdf("mags-all/5-SNPs/dndscv/results/plot/snp_dnds_enrichnet.pdf", height = 9, width = 13)
...
```{r fig3}
# Merge plots using patchwork
fig_3a <- wrap_plots(plot_spacer(), snp_plot_psgenes, snp_plot_estim_phy,
                     snp_plot_estim_bin, plot_spacer(),
                     nrow = 1, ncol = 5, widths = c(0.05, 0.75, 0.55, 0.65, 0.15))
fig_3b <- wrap_plots(snp_plot_nsgenes, snp_plot_nsgensub, snp_plot_kos,
                     nrow = 1, ncol = 3, widths = c(0.85, 0.85, 1))
fig_3 <- wrap_plots(fig_3a, fig_3b, nrow = 2, heights = c(11.5, 10.5)) +
  plot_annotation(tag_levels = list(c("a", "", "b", "c", "d", "e", "f"))) &
  theme(plot.tag =  element_text(face = "bold"))
ggsave("mags-all/fig_3.pdf", fig_3, height = 22, width = 28)
#ggsave("mags-all/fig_3.png", fig_3, height = 22, width = 28)
rm(fig_3a, fig_3b) #clean memory

#sup 3
fig_s3 <- wrap_plots(snp_plot_psbins, widths = c(1)) +
  plot_annotation(tag_levels = list(c("a", "", ""))) &
  theme(plot.tag =  element_text(face = "bold"))
ggsave("mags-all/fig_s3.pdf", fig_s3, height = 9, width = 13)
#ggsave("mags-all/fig_s3.png", fig_s3, height = 9, width = 13)
```
