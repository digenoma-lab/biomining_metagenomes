...
##use wrap_plots() to correct axis line/labels ??
ggsave("mags-all/fig_s1.pdf", fig_s1, height = 8, width = 16)
#ggsave("mags-all/fig_s1.png", fig_s1, height = 8, width = 16)
```
...
rm(snp_plot_psbins2, snp_plot_psbins3)
...
# Plot PS genes mutations per type
...
# Merge NS genes to unique products
...
# Lollipop plot of NS genes dN/dS x-axis range 0-2, y-axis PS genes vs selected CDS
snp_plot_nsgensub <- ggdotchart(snp_nsgenes_sub, x = "product", y = "dnds", color = "pathway",
                              xlab = "NS gene product", ylab = "Mean dN/dS", y.text.col = FALSE,
                              rotate = TRUE, palette = snp_nsgenes_sub_col,
...
# Global gene estimation of dn/ds (<1 & >1) per number of genes and phylum
snp_estim <- snp_selcv %>%
  dplyr::select(gene_name, wmis_cv, pmis_cv) %>%
...
snp_estim <- left_join(snp_estim, snp_dnds, by = c("bin")) %>%
  dplyr::select(-c("contig", "num", "genus", "species"))
...
snp_estim_phy <- snp_estim %>%
  ...
  ungroup() %>%
  dplyr::mutate(dnds_glob = dnds_sum / snps) %>% #divide by total genes and top 3 phyla
...
snp_estim_sort_phy <- snp_estim_phy %>%
  ...
  dplyr::arrange(desc(n))
snp_estim_sort_phy$color <- rep("#4c6194", 3)
...
# Load lists of PS and NS genes (with KEGG KOs from KOfamScan)
...
#merge dataframe and filter
snp_kos <- bind_rows(snp_kops, snp_kons)
snp_kos <- snp_kos %>%
  dplyr::filter(sum_ko > 10)
...
# Filter genome bin with PS selection
snp_cirmag <- snp_estim %>% dplyr::filter(grepl("S15.49", bin))

# Set genomic attributes: gc, contigs
...
#NODE_4971_length_14023_cov_250.866767	2403	4742	BNHKIBDC_01907
...
# Circos plot
circos.clear()
pdf("mags-all/5-SNPs/dndscv/results/plot/snp_dnds_circos.pdf", height = 5, width = 5)
...
circos.track(snp_cirmagname, ylim = snp_cirsize, panel.fun = function(x, y) {
  ...
  text(0, -0.08, paste("phylum Actinobacteriota | genus Acidithrix", 
...
}, bg.col = "#e2edf2", bg.border = FALSE, track.height = 0.06)
circos.genomicLabels(snp_cirgenes, labels.column = 6, cex = 0.35, line_lwd = 0.5,
...
```{r fig3}
# Merge plots using patchwork
fig_3a <- wrap_plots(plot_spacer(), snp_plot_psgenes, snp_plot_estim_phy, plot_spacer(),
                     nrow = 1, ncol = 4, widths = c(0.1, 0.85, 0.5, 0.4))
fig_3b <- wrap_plots(snp_plot_estim_bin, snp_plot_nsgenes, snp_plot_nsgensub,
                     nrow = 1, widths = c(1, 0.75, 1))
fig_3 <- wrap_plots(fig_3a, fig_3b, nrow = 2) +
  plot_annotation(tag_levels = list(c("a", "", "b", "c", "d", "e"))) &
  theme(plot.tag =  element_text(face = "bold"))
ggsave("mags-all/fig_3.pdf", fig_3, height = 20, width = 24)
#ggsave("mags-all/fig_3.png", fig_3, height = 20, width = 24)
rm(fig_3a, fig_3b)

#sup 3
fig_s3 <- wrap_plots(snp_plot_psbins, snp_plot_kos, nrow = 1, widths = c(1, 0.5)) +
  plot_annotation(tag_levels = list(c("a", "", "", "b"))) &
  theme(plot.tag =  element_text(face = "bold"))
ggsave("mags-all/fig_s3.pdf", fig_s3, height = 10, width = 26)
ggsave("mags-all/fig_s3.png", fig_s3, height = 10, width = 26)
```
