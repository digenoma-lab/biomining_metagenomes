...
plot_md_top_phyla <- ggplot(mags_md_top_phy, aes(x = reorder(phy_top, freq), y = freq)) +
  ...
  theme_classic() +
  coord_flip()
ggsave("plot_md_top_phyla.pdf", plot_md_top_phyla, height = 7, width = 6)
...
plot_depth_spec <- ggplot(mags_depth_spec) +
  ...
  labs(x = "Known biomining species", y = "Count") +
...
plot_depth_spec_eco <- ggplot(mags_depth_spec_eco, aes(x = reorder(ecosystem, ratio),
  ...
  labs(x = "Ecosystem", y = "Number of kSGBs / Number of MAGs") +
...
plot_depth_sgbs_biom <- ggplot(mags_depth_sgbs_biom,
  ...
  labs(x = "Phylum", y = "Number of biomining species") +
  theme_classic() +
...
plot_tree_s15_phyla <- plot_tree_s15 +
  geom_tippoint(aes(color = phylum), na.rm = TRUE, size = 1, show.legend = FALSE) +
  geom_tiplab(aes(label = phylum, color = phylum),
              size = 1.5, hjust = -0.1, show.legend = FALSE) +
  scale_colour_discrete(breaks = c('c', 'd'), na.value = NA) +
  scale_colour_manual(values = tree_colors, na.translate = FALSE) +
  theme()
  #+guides(color = guide_legend(title = "Phylum", ncol = 1, order = 1))
...
plot_tree_s15_md <- plot_tree_s15_phyla + new_scale_fill() + new_scale_color() +
  geom_fruit(data = tree_qc_df, geom = geom_bar,
             ...
             grid.params = list(linetype = 1, size = 0.1),
             axis.params = list(axis = "x", text.size = 1.8, vjust = 1, nbreak = 3),
             pwidth = 0.15, offset = 0.35) +
  scale_fill_gradient(low = "#1F4060", high = "#4CA8F6",
  ...
  scale_fill_gradient(low = "#6A60A8", high = "#C4C2DE",
                      guide = guide_legend(keyheight = 0.4, keywidth = 0.7,
  ...
  geom_fruit(data = tree_qc_df, geom = geom_bar,
             ...
             axis.params = list(axis = "x", text.size = 1.8, vjust = 1, nbreak = 3),
             pwidth = 0.15, offset = 0.1) +
  scale_fill_gradient(name = "Strain Heterogeneity",
                      low = "#C9543F", high = "#E9bAB2",
                      guide = guide_legend(keyheight = 0.4, keywidth = 0.7,
                                           reverse = TRUE, order = 3)) +
  new_scale_fill() + new_scale_color() +
  geom_fruit(data = tree_qc_df, geom = geom_bar,
             ...
             axis.params = list(axis = "x", text.size = 1.8, vjust = 1, nbreak = 1),
             pwidth = 0.15, offset = 0.1) +
  scale_fill_gradient(name = "Predicted Genes",
                      low = "#279854", high = "#A9DEDE",
                      guide = guide_legend(keyheight = 0.4, keywidth = 0.7,
                                           reverse = TRUE, order = 4)) +
  theme(legend.title = element_text(size = 8),
        legend.text = element_text(size = 7),
        legend.key.height = unit(3.5, units = "mm"))
ggsave("plot_biom_MAGs_tree_md.pdf", plot_tree_s15_md, height = 7, width = 10)
```

## Figure 1

```{r fig1}
# Merge generated figures using patchwork
fig1_a <- {
  (plot_tree_s15_md | plot_depth_spec_eco)
  } + plot_layout(ncol = 2, widths = c(1, 0.5))
ggsave("figures/fig1_a.pdf", fig1_a, height = 7, width = 14)

fig1_b <- {
  (plot_mags_qc / plot_depth_sgbs_biom) |
    plot_depth_sgbs_both | plot_depth_kggbs_biom
  } + plot_layout(ncol = 3, widths = c(0.5, 2, 0.75))
ggsave("figures/fig1_b.pdf", fig1_b, height = 7, width = 14)

##fig1
fig1 <- (fig1_a / fig1_b) +
  plot_annotation(tag_levels = list(c("a", "e", "b", "c", "d", "", "f")))
ggsave("figures/fig1.pdf", fig1, height = 14, width = 14)
rm(fig1_a, fig1_b)

##fig2
```

## Functional Annotation: KEGG KOs from KOfamScan and Anvio Kofam
...
kegg_paths <- kegg_kos %>%
  ...
  filter(grepl("iron|sulf|soxr|copper", Definition, ignore.case = TRUE) == TRUE) %>%
  select(KO, Definition)

## FUNCTIONS ##
## Function that returns the pathway based on the definitions
...
## Function to define a list of key metabolism
...
# Save pathways and metabolism to dataframe
...
# Filter matrix to select key pathways
kegg_rows <- rowSums(kegg_annot)
#summary(kegg_rows)
#table(kegg_rows)
kegg_filt <- kegg_annot[row.names(kegg_annot) %in% kegg_paths$KO, ]

# Normalize filtered rows by z-score
kegg_filt_norm <- apply(kegg_filt, 1,
                        norm <- function(x){return (x - mean(x)) / sd(x)}) %>% t()
...
kegg_hm_rows_path <- list(Def = kegg_paths$Pathway, Pathways = kegg_hm_rows_path)

kegg_hm_rows_ann_path <- HeatmapAnnotation(Pathways = kegg_paths$Pathway,
                                           col = kegg_hm_rows_path, border = FALSE,
                                           show_annotation_name = FALSE,
                                           show_legend = TRUE, which = "row")

# Annotate rows: KO metabolism
kegg_hm_rows_metab <- setNames(brewer.pal(length(unique(kegg_paths$Metabolism)), "Paired"), unique(kegg_paths$Metabolism)) #Set3
kegg_hm_rows_metab <- sample(kegg_hm_rows_metab)
kegg_hm_rows_metab <- list(Met = kegg_paths$Metabolism, Metabolism = kegg_hm_rows_metab)

kegg_hm_rows_ann_metab <- HeatmapAnnotation(Metabolism = kegg_paths$Metabolism,
                                            col = kegg_hm_rows_metab, border = FALSE,
                                            show_annotation_name = FALSE,
                                            show_legend = TRUE, which = "row")

# Annotate cols: separate biomining MAGs from other soil SMAG
...
### 2. Read output file from Anvio Kofam
anvio_annot <- read.csv("/mnt/beegfs/home/mrojas/DiGenomaLab/Systemix/analysis/kegg/results/anvio_kofam_hits.tsv", sep = "\t")

# Save KO definitions and set row names
#anvio_kos <- data.frame(KO = anvio_annot$ko,
#                        Modules = anvio_annot$modules_with_ko,
#                        Definition = anvio_annot$ko_definition)
#anvio_annot <- anvio_annot %>% select(-c("modules_with_ko", "ko_definition"))
#anvio_annot <- data.frame(anvio_annot, row.names = 1)
#
## Set definition for selected pathways
#anvio_paths <- anvio_kos %>%
#  filter(grepl("iron|sulfur|copper", Definition, ignore.case = TRUE) == TRUE) %>%
#  select(KO, Definition)
##which(anvio_paths$KO %in% kegg_paths$KO)
#
#anvio_def_paths <- vector()
#for (i in 1:nrow(anvio_paths)) {
#	anvio_def_paths <- append(anvio_def_paths, setKODefPathway(anvio_paths$Definition[i]))
#}
#anvio_paths['Pathway'] <- anvio_def_paths
#
## Filter matrix to select key pathways
## Preprocess matrix.. (filter KOs per row)
#anvio_rows <- rowSums(anvio_annot)
#summary(anvio_rows)
#table(anvio_rows)
#anvio_filt <- anvio_annot[row.names(anvio_annot) %in% anvio_paths$KO, ]
```
...
# Note: relevant cycles (iron, sulfur, copper) have low representation in row sums,
# so during filtration we need to consider them within the range of values.
# For example, rows with low variances could be kept but they are also too many..

# Preprocess matrix
kegg_pca_mat <- kegg_annot[kegg_rows > 1 & kegg_rows < round(summary(kegg_rows)[4], 0), ]

# Merge KO definitions
kegg_pca_kos <- data.frame(KO = row.names(kegg_pca_mat))
...
kegg_pca_mat <- apply(kegg_pca_mat, 1,
                      norm_pca <- function(x){return (x - mean(x)) / sd(x)}) %>% t()

# PCA based on SVD
set.seed(12345)
kegg_pca <- prcomp(kegg_pca_mat, center = FALSE, scale. = FALSE)
...
kegg_pca_screeplot <- ggplot(kegg_pca_scree_df, aes(x = PC, y = Var)) +
  ...
  labs(title = "KEGG KOs (KOfamScan) Scree Plot", x = "PC", y = "Variance Explained") +
  theme_minimal()
ggsave("plot_annot_pca_kofam_screeplot.pdf", kegg_pca_screeplot, height = 4, width = 6)
...
# Set colors for ecosystem (standarize**)
kegg_colors_eco <- setNames(brewer.pal(length(unique(kegg_mags$ecosystem)), "Paired"), unique(kegg_mags$ecosystem))
kegg_colors_eco <- sample(kegg_colors_eco)


### PCAtools ###
set.seed(11122)
kegg_pca_obj <- pca(kegg_pca_mat, center = FALSE, scale = FALSE,
...
kegg_pca_obj_biplot <- biplot(kegg_pca_obj, showLoadings = FALSE, sizeLoadingsNames = FALSE,
                              ...
                              colkey = kegg_colors_eco, ellipse = TRUE, ylim = c(-14, 12))
...
kegg_pca_obj_pairs <- pairsplot(kegg_pca_obj, colby = "ecosystem",
                                colkey = kegg_colors_eco, pointSize = 1,
...
# Subset to cycles of interest (3):
# 1. get definitions and metabolism from kegg_paths
# 2. merge df with counts per ecosystem
# 3. remove biomining KOs with low counts (ie. sulfur: <= 5)
# 4. subset df with kept KOs
#todo: split definition in two columns by " [EC:"
kegg_bubble_copper <- kegg_paths %>%
  filter(Pathway == "Copper") %>%
  select(KO, Definition, Metabolism)
kegg_bubble_copper <- left_join(kegg_bubble_copper, kegg_bubble, by = c("KO"))
kegg_bubble_keep <- kegg_bubble_copper %>%
  filter(ecosystem == "Biomining" & counts > 1) %>%
  select(KO)
...
kegg_bubble_keep <- kegg_bubble_iron %>%
  filter(ecosystem == "Biomining" & counts > 2) %>%
  select(KO)
kegg_bubble_iron <- kegg_bubble_iron[kegg_bubble_iron$KO %in% kegg_bubble_keep$KO, ] %>%
  filter(!KO == "K02014") #remove outlier with counts > 500

kegg_bubble_sulfur <- kegg_paths %>%
  filter(Pathway == "Sulfur") %>%
  select(KO, Definition, Metabolism)
kegg_bubble_sulfur <- left_join(kegg_bubble_sulfur, kegg_bubble, by = c("KO"))
kegg_bubble_keep <- kegg_bubble_sulfur %>%
  filter(ecosystem == "Biomining" & counts >= 5) %>%
  select(KO)
kegg_bubble_sulfur <- kegg_bubble_sulfur[kegg_bubble_sulfur$KO %in% kegg_bubble_keep$KO, ]

# Bubble plot
kegg_bubble_plot <- ggplot(kegg_bubble_iron,
                           aes(x = ecosystem, y = Definition,
                               size = counts, color = ecosystem)) +
  geom_point(alpha = 0.7) +
  scale_color_manual(values = kegg_colors_eco, guide = "none") +
  labs(title = "", x = "Ecosystem", y = "KEGG KO Definitions") +
  theme_classic() +
  theme(legend.title = element_text(size = 10),
        legend.position = "right",
        axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1))
ggsave("plot_annot_bubble_kofam_copper.pdf", kegg_bubble_plot, height = 7, width = 9)
ggsave("plot_annot_bubble_kofam_iron.pdf", kegg_bubble_plot, height = 7, width = 7)
ggsave("plot_annot_bubble_kofam_sulfur.pdf", kegg_bubble_plot, height = 9, width = 9)
```
