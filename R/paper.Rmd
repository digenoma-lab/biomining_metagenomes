...
ggsave("fig_s1.pdf", fig_s1, height = 8, width = 16)
#ggsave("fig_s1.png", fig_s1, height = 8, width = 16)
```
...
anova_boxplot_cu <- ggboxplot(kegg_bubble_cu, x = "ecosystem", y = "count_norm",
  ...
  geom_hline(yintercept = mean(kegg_bubble_cu$count_norm),
...
anova_boxplot_fe <- ggboxplot(kegg_bubble_fe, x = "ecosystem", y = "count_norm",
  ...
  geom_hline(yintercept = mean(kegg_bubble_fe$count_norm),
             linetype = 2, linewidth = 0.2) +
...
anova_boxplot_s <- ggboxplot(kegg_bubble_s, x = "ecosystem", y = "count_norm",
  ...
  geom_hline(yintercept = mean(kegg_bubble_s$count_norm),
             linetype = 2, linewidth = 0.2) +
...
ggsave("fig_2.pdf", fig_2, height = 14, width = 18)
#ggsave("fig_2.png", fig_2, height = 14, width = 18) vjust=120
```
...
# Buildref testing for bacteria.. (1 bin)
...
head(snp_dndsout$annotmuts)
print(snp_dndsout$globaldnds) #driver globals

# Save dN/dS per-gene matrix
#driver discovery or genes under positive selection (PS)
snp_selcv <- snp_dndsout$sel_cv
rm(snp_dndsout)

# Group global dN/dS ratios:
#https://github.com/im3sanger/dndscv/issues/63
snp_selcv %>% dplyr::filter(pmis_cv < 0.05) %>% nrow() #n=105328
...
fx_dndsTaxaBin <- function(b) {
  t <- mag_samples %>%
    dplyr::mutate(bin = paste0(mag_name, ".", mag_id)) %>%
    dplyr::filter(bin == b) %>%
    dplyr::select(phylum, genus, species, predicted_genes)
  return(t)
}
...
# Function that gets dndscv stats for NS genes
...
snp_taxabins <- tibble(phylum = character(), genus = character(), species = character(),
                       completeness = numeric(), contamination = numeric())
...
if (file.exists("mags-all/5-SNPs/dndscv/results/kofam/ps_genes_ids.txt") == FALSE) {
  write.table(snp_psgenes_sub %>% dplyr::select(name) %>% distinct(name), "mags-all/5-SNPs/dndscv/results/kofam/ps_genes_ids.txt", sep = "\t", quote = FALSE, row.names = FALSE, col.names = FALSE)
}

# Barplot with global dN/dS ratios
...
# Plot global driver genes by species
snp_plot_psbins <- ggplot(snp_dnds, aes(x = num, y = reorder(bin, num), fill = bin)) +
  geom_bar(stat = "identity", fill = "#E31A1C") +
  geom_text(aes(label = num), vjust = 0.5, hjust = -0.5, size = 2.5) +
  labs(x = "Number of genes under PS", y = "Bin") +
  scale_x_continuous(limits = c(0, 255)) +
  theme_classic() +
  theme(strip.background = element_blank()) +
  facet_grid(.~species)
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_psbins.pdf", snp_plot_psbins, height = 8.5, width = 9.5)
...
snp_plot_nsgensub <- ggdotchart(snp_nsgenes_sub, x = "product", y = "dnds", color = "pathway",
                              xlab = "NS gene product", ylab = "dN/dS", y.text.col = FALSE,
                              rotate = TRUE, palette = snp_nsgenes_sub_col,
                              sorting = "descending", ggtheme = theme_pubr()) +
  scale_y_continuous(limits = c(0, 1)) +
  geom_hline(yintercept = mean(snp_nsgenes$wmis_cv), linetype = 2, linewidth = 0.2) +
  geom_hline(yintercept = 1, linetype = 1, linewidth = 0.2)
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_nsgenespath.pdf", snp_plot_nsgensub, height = 9, width = 10)

# Count total SNPs per mutation type for phylum
snp_topphyla <- snp_psgenes %>%
  dplyr::select(gene_name, n_syn, n_mis, n_non, bin, name) %>%
  dplyr::mutate(snps = n_syn + n_mis + n_non)
snp_topphyla <- snp_topphyla %>%
  dplyr::group_by(bin) %>%
  dplyr::summarise(sum_nsyn = sum(n_syn),
                   sum_nmis = sum(n_mis),
                   sum_nnon = sum(n_non))
snp_topphyla <- left_join(snp_topphyla, snp_dnds, by = c("bin")) %>%
  dplyr::select(-c("num", "genus", "species")) %>%
  pivot_longer(cols = c(sum_nsyn, sum_nmis, sum_nnon, predicted_genes),
               names_to = "muts", values_to = "nmuts")
#top 3 phyla and merge
snp_topphyla <- snp_topphyla %>%
  dplyr::filter(phylum == "Proteobacteria" | phylum == "Acidobacteriota" |
                  phylum == "Actinobacteriota") %>%
  dplyr::group_by(phylum, muts) %>%
  dplyr::summarise(tot = sum(nmuts), .groups = "drop")

# Barplot #snps/#genes
snp_plot_topphy <- ggplot(snp_topphyla, aes(x = reorder(phylum, -tot), y = tot, fill = muts)) +
  geom_col(position = position_stack(reverse = TRUE)) +
  #geom_text(aes(label = tot), position = position_stack(reverse = TRUE), size = 2, vjust = 0.5) +
  scale_fill_manual(name = "", labels = c("gen", "mis", "non", "syn"),
                    values = c("#cba993", "#5db4b2", "#b687b6", "#2374a4")) +
  scale_y_continuous(limits = c(0, 70000)) +
  labs(x = "Phylum", y = "Number of PS mutations per type") +
  theme_classic() +
  theme(legend.position = c(0.85, 0.9),
        strip.placement = "outside")
ggsave("mags-all/5-SNPs/dndscv/results/plot/plot_dnds_topphyla.pdf", snp_plot_topphy, height = 7, width = 5)

### ###
# Merge PS and NS gene names
#snp_gmerge <- bind_rows(snp_psgenes, snp_nsgenes)
#snp_gmerge <- snp_gmerge %>% dplyr::mutate(pathway = ifelse(is.na(pathway), "PS", pathway))
#snp_color_gm <- c("#ddad3b", "#4f4fad", "#6a9b85", "#ca6627", "#8c2421", "black")
### ###
...
# Reduce to unique KOs, saving scores (eq. to foldchange?)
...
```{r fig3}
{ { anova_boxplot_cu / anova_boxplot_fe +
    plot_layout(ncol = 1, widths = c(0.75)) } | anova_boxplot_s / kegg_bubbles_plot }
# Merge plots using patchwork
fig_3 <- { { snp_plot_dndsglob + snp_plot_psgenes + snp_plot_topphy +
    plot_layout(ncol = 3, widths = c(0.25, 0.75, 0.75)) } |
    snp_plot_nsgenes + snp_plot_nsgensub } +
  plot_annotation(tag_levels = list(c("a", "b", "c", "d", "e"))) &
  theme(plot.tag =  element_text(face = "bold"))
ggsave("fig_3.pdf", fig_3, height = 14, width = 18)

#fig_s3 <- {snp_plot_psbins}
```
