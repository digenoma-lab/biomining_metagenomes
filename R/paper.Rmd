...
# Inspect and select sample S15
#filter(mags_qc, grepl("\\-S15\\.", bin)) %>% dplyr::select(2:3) %>% summary()
mag_samples <- dplyr::filter(gtdb_taxonomy, grepl("\\-S15\\.", genome))
...
# Save metadata for sample S15
write.table(mag_samples, "4-mags-smag/mags_mining_S15.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
...
### 1.1: In-depth biomining MAGs vs SMAG
...
# Change ecosystem from "biomining" to "mining"
mag_samples <- mag_samples %>% dplyr::mutate(ecosystem = "Mining")
mags_metadata_bac <- mags_metadata_bac %>%
  dplyr::mutate(ecosystem = case_when(ecosystem == "Biomining" ~ "Mining", TRUE ~ ecosystem))
...
plot_depth_top_phyla <- mags_md_top_phy %>%
  ...
  labs(x = "Number of MAGs", y = "Phylum") +
  scale_x_continuous(limits = c(0, 10000)) +
  theme_classic() +
  theme(axis.text = element_text(size = 15),
        axis.title = element_text(size = 17))
ggsave("article_profiling/4-mags-smag/1-plots-taxonomy/plot_md_top_phyla.pdf", plot_depth_top_phyla, height = 8, width = 6)
...
mags_colors_eco <- c("#1F78B4", "#FF7F00", "#B2DF8A", "#A6CEE3", "#6A3D9A",
                     "#33A02C", "#E31A1C", "#CAB2D6", "#FDBF6F", "#FB9A99")

# Which bacteria from mining sites are annotated as known?
mags_depth_spec_biom <- mags_metadata_bac %>%
  dplyr::select(mag_name, family, genus, species, ecosystem) %>%
  dplyr::filter(species != "" & ecosystem == "Mining")
...
plot_depth_spec <- ggplot(mags_depth_spec) +
  ...
  labs(x = "", y = "Number of kSGBs from mining") +
  scale_fill_manual(values = "#78acc2") +
  coord_flip() +
  theme_classic() +
  theme(legend.title = element_blank(),
        legend.position = "top", #c(0.88, 0.12),
...
mags_depth_spec_eco <- mags_depth_spec_eco %>%
  dplyr::mutate(mags = replace(mags, ecosystem == "Mining", 1)) %>%
...
plot_depth_spec_eco <- mags_depth_spec_eco %>%
  ...
  labs(x = "Ecosystem", y = "Number of kSGBs/samples") +
...
# And unknown/known SGBs for mining?
mags_depth_sgbs_biom <- mags_metadata_bac %>%
  dplyr::filter(ecosystem == "Mining") %>%
  dplyr::group_by(phylum, species) %>%
  dplyr::summarise(n_spe = n()) %>%
  ungroup() %>%
  dplyr::group_by(phylum) %>%
...
plot_depth_sgbs_biom <- mags_depth_sgbs_biom %>%
  ...
  labs(x = "Phylum", y = "Mining species") +
  coord_flip() +
  theme_classic() +
...
ggsave("article_profiling/4-mags-smag/1-plots-taxonomy/plot_md_uSGBs_eco.pdf", plot_depth_usgbs, height = 11, width = 17)

# What about the annotated bins at genus level (kGGBs) from mining?
mags_depth_kggbs_biom <- mags_metadata_bac %>%
  dplyr::select(mag_name, mag_id, phylum, family, genus, ecosystem) %>%
  dplyr::filter(genus != "" & ecosystem == "Mining") #nrow=21
...
plot_depth_kggbs_biom <- mags_depth_kggbs_biom %>%
  ...
  scale_fill_manual(values = mags_colors_eco) +
  labs(x = "% kGGBs", y = "kGGBs from mining") +
  theme_classic() +
  theme(legend.title = element_blank(),
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 14))
...
#  #slice(n()) %>%
#  ungroup()
#unique(mags_md_unk_gen2[mags_md_unk_gen2[, "ecosystem"] == "Mining", "class"])
#
##unique genus from mining: Acidithrix, Ferrithrix, SCQM01
...
# Select sample S15 and export to file
mags_qc <- mags_qc %>% filter(grepl("Refined-S15", bin))
write.table(mags_qc, "article_profiling/2-statistics/mags_S15.txt", sep = "\t", quote = FALSE, row.names = FALSE)
...
##export to file parsed gtdb_taxonomy 1-taxonomy/gtdbtk_mags_S15.tsv
##checkm_parsed_bins.tsv 2-statistics/checkm_mags_S15.tsv
...
```{r abund}
# Read matrix of raw abundances
abu_depths <- read.csv("article_profiling/3-abundances/raw/bin_refined_depths_summary.tsv", sep = "\t")

# Filter list to S15 sample
abu_depths <- abu_depths %>%
  dplyr::filter(!grepl("Unbinned", bin)) %>%
  dplyr::filter(grepl("S15", bin)) %>%
  dplyr::mutate(bin = str_replace_all(bin, ".fa", "")) %>%
  dplyr::select(bin, S15)

# Add phylum
abu_depths <- left_join(abu_depths, dplyr::select(gtdb_taxonomy, label, phylum),
                        by = join_by(bin == label))

# Dotplot of global abundances by MAG size
abu_plot_depth <- abu_depths %>%
  ...
  ggplot(aes(x = S15, y = reorder(bin, S15), color = phylum)) +
  geom_point(aes(size = S15), alpha = 0.7) +
  scale_size(range = c(0, 10)) +
  scale_color_manual(values = tree_colors) +
  scale_x_log10(expand = c(0.1, 0)) + #add x-axis space
  scale_y_discrete(expand = c(0.025, 0)) + #add y-axis space
  guides(color = guide_legend(title = "Phylum", ncol = 1), size = "none") +
  labs(x = "Abundance (mean coverage)", y = "MAG") +
  theme_classic() +
  theme(legend.text = element_text(size = 11),
        legend.title = element_text(size = 13),
        axis.text = element_text(size = 12),
        axis.title = element_text(size = 14))
ggsave("article_profiling/3-abundances/plots/plot_abu_rank.pdf", abu_plot_depth, height = 8, width = 6)

# Define top phyla
abu_top_phy <- c("Acidobacteriota", "Actinobacteriota", "Proteobacteria")

# Boxplot of abundances by top phyla
abu_plot_depth.2 <- abu_depths %>%
  dplyr::filter(phylum %in% abu_top_phy) %>%
  dplyr::mutate(phylum = factor(phylum, levels = c("Actinobacteriota", "Acidobacteriota", "Proteobacteria"))) %>% #sort
  ggplot(aes(x = phylum, y = S15, color = phylum)) +
  geom_quasirandom(aes(size = S15), alpha = 0.7) +
  scale_size(range = c(0, 10)) +
  scale_y_log10() +
  geom_boxplot(fill = NA, outlier.size = 0, outliers = FALSE) +
  scale_color_manual(values = c("#98d277", "#a6cee3", "#b15928")) +
  guides(size = "none", color = "none") +
  stat_compare_means(method = "anova", label.y = 3.8, size = 4.5) + #pos by log10
  stat_compare_means(method = "t.test", label = "p.format", ref.group = ".all.", vjust = 0.25,
                     comparisons = list(c("Actinobacteriota", "Acidobacteriota"),
                                        c("Actinobacteriota", "Proteobacteria"),
                                        c("Acidobacteriota", "Proteobacteria")),
                     label.y = c(3.3, 3.45, 3.64), bracket.size = 0.25, size = 4) +
  labs(x = "Phylum", y = "Abundance (mean coverage)") +
  theme_classic() +
  theme(axis.text = element_text(size = 12),
        axis.title = element_text(size = 14))
ggsave("article_profiling/3-abundances/plots/plot_abu_phy.pdf", abu_plot_depth.2, height = 7, width = 5)
```

## Figures 1-2 & supplementary 1

```{r fig1}
# Merge plots using patchwork
fig_1a <- wrap_plots(plot_mags_qc, plot_depth_sgbs_biom, plot_tree_s15_md,
                     design = "ACCC\nBCCC", widths = c(0.6, 1)) +
  plot_annotation(tag_levels = list(c("a", "b", "c"))) &
  theme(plot.tag = element_text(size = 16, face = "bold"))
ggsave("article_profiling/fig_1a.pdf", fig_1, height = 8.5, width = 13)
#ggsave("article_profiling/fig_1a.png", fig_1, height = 8.5, width = 13)
fig_1b <- wrap_plots(abu_plot_depth, abu_plot_depth.2, widths = c(0.65, 1)) +
  plot_annotation(tag_levels = list(c("d", "e"))) &
  theme(plot.tag = element_text(size = 16, face = "bold"))
ggsave("article_profiling/fig_1b.pdf", fig_1b, height = 8, width = 11)
#ggsave("article_profiling/fig_1b.png", fig_1b, height = 8, width = 11)
#compose
fig_1 <- wrap_plots(fig_1a / fig_1b) +
  plot_annotation(tag_levels = list(c("a", "b", "c", "d", "e"))) &
  theme(plot.tag = element_text(size = 16, face = "bold"))
ggsave("article_profiling/fig_1.pdf", fig_1, height = 16, width = 13)
ggsave("article_profiling/fig_1.png", fig_1, height = 16, width = 13)
rm(fig_1a, fig_1b) #clean memory

fig_2 <- wrap_plots(plot_depth_sgbs_both, plot_depth_spec_eco,
                    plot_depth_kggbs_biom, design = "AAABBCC") +
  plot_annotation(tag_levels = list(c("a", "", "b", "c"))) &
  theme(plot.tag = element_text(size = 18, face = "bold"))
ggsave("article_profiling/fig_2.pdf", fig_2, height = 8, width = 16)
ggsave("article_profiling/fig_2.png", fig_2, height = 8, width = 16)

#sup 1
fig_s1 <-  { plot_depth_top_phyla |
    (plot_depth_spec / plot_depth_usgbs + plot_layout(heights = c(0.15, 0.85))) } +
  plot_layout(ncol = 2, widths = c(0.25, 1)) +
  plot_annotation(tag_levels = list(c("a", "b", "c"))) &
  theme(plot.tag = element_text(size = 18, face = "bold"))
fig_s1[[2]][[1]] <- fig_s1[[2]][[1]] + theme(axis.line.y.left = element_blank(),
                                             axis.ticks.y.left = element_blank())#,
                                             #axis.text.y = element_text(hjust = 2.2))
fig_s1[[2]][[2]] <- fig_s1[[2]][[2]] + theme(axis.title.y = element_text(vjust = 0))
ggsave("article_profiling/fig_s1.pdf", fig_s1, height = 15, width = 24)
ggsave("article_profiling/fig_s1.png", fig_s1, height = 15, width = 24)
```

## Section 3: Functional Annotation

### 3.1: Process KEGG KOs

```{r processkos}
# Read output matrix from KOfamScan
kegg_annot <- read.csv("article_profiling/4-mags-smag/kofamscan_hits.tsv", sep = "\t", check.names = FALSE)
...
#kegg_mags %>% dplyr::filter(ecosystem == "Mining") %>% dplyr::summarise(total = sum(length))
...
pdf("article_profiling/4-mags-smag/2-plots-annotation/heatmap/plot_annot_heatmap.pdf", height = 9, width = 9)
kegg_hm_plot <- Heatmap(kegg_hm_data, name = "Abundance",
...
# (loop runtime ~2 mins for rows in kegg_paths)
...
#explore
#kegg_bubble %>% dplyr::group_by(ecosystem) %>% dplyr::summarise(n = sum(count_norm)) %>% dplyr::arrange(desc(n))
...
kegg_bubble_keep <- kegg_bubble_cu %>%
  dplyr::filter(ecosystem == "Mining" & count_norm > 0) %>%
  dplyr::select(KO)
...
kegg_bubble_keep <- kegg_bubble_fe %>%
  dplyr::filter(ecosystem == "Mining" & count_norm > 0.01) %>%
  dplyr::select(KO)
...
kegg_bubble_keep <- kegg_bubble_s %>%
  dplyr::filter(ecosystem == "Mining" & count_norm > 0.05) %>%
  dplyr::select(KO)
...
anova_boxplot_fe <- ggboxplot(kegg_bubble_fe, x = "ecosystem", y = "count_norm",
  ...
  labs(x = "Ecosystem", y = "Iron proteins (KOs/Mb)") +
  rotate_x_text(angle = 45) + ylim(c(0, 0.4)) +
  stat_compare_means(method = "anova", label.y = 0.4, size = 6) + #add global annova p-value
...
anova_boxplot_s <- ggboxplot(kegg_bubble_s, x = "ecosystem", y = "count_norm",
  ...
  labs(x = "Ecosystem", y = "Sulfur proteins (KOs/Mb)") +
  rotate_x_text(angle = 45) + ylim(c(0, 0.8)) +
  stat_compare_means(method = "anova", label.y = 0.8, size = 6) + #add global annova p-value
  stat_compare_means(label = "p.signif", method = "t.test", ref.group = ".all.",
                     hide.ns = TRUE, label.y = 0.75, size = 8) +
  theme(axis.text = element_text(size = 15),
        axis.title = element_text(size = 17))
...
kegg_bubbles_plot <- kegg_bubble %>%
  ...
  theme(legend.position = "top",
        ...
        axis.text.x = element_text(size = 16, angle = 60, vjust = 1, hjust = 1),
        axis.text.y = element_text(size = 16),
        axis.title.x = element_text(size = 18))
ggsave("article_profiling/4-mags-smag/2-plots-annotation/bubble/plot_annot_bubbles.pdf", kegg_bubbles_plot, height = 8, width = 13)
...
```{r fig3}
# Merge plots using patchwork
fig_3 <- wrap_plots(anova_boxplot_cu, anova_boxplot_fe, anova_boxplot_s, design = "AABBCC") +
  plot_annotation(tag_levels = list(c("a", "b", "c"))) &
  theme(plot.tag = element_text(size = 22, face = "bold"))
ggsave("article_profiling/fig_3.pdf", fig_3, height = 7, width = 20)
ggsave("article_profiling/fig_3.png", fig_3, height = 7, width = 20)
```
...
## --- filtered VCF --- ##
# Load matrix with dN/dS: driver discovery or genes under positive selection (PS)
load("article_profiling/5-SNPs/dndscv/run/selcv_s15_filt.rda") #146,932 genes

# Load driver globals
load("article_profiling/5-SNPs/dndscv/run/global_s15_filt.rda")
...
rm(snp_refcds, snp_bins, snp_genids, snp_gene2contig, snp_contig2bin, snp_genenames, i) #clean memory

# All MAGs has genes under PS?: No
length(unique(snp_psgenes$bin))
# Which?: S15.19
mags_qc[!(mags_qc$bin %in% snp_psgenes$bin), ]
...
snp_psgenes_sub <- snp_psgenes %>%
  ...
  dplyr::mutate(path = case_when(gene_name == "NEBJFEFK_01910" ~ "S", TRUE ~ "")) %>%
...
snp_plot_psbins <- snp_dnds %>%
  ...
  labs(x = "Number of genes under PS", y = "MAG") +
...
snp_plot_psgenes <- snp_psgenes_sub %>%
  ...
  geom_point(alpha = 0.7, size = 3, color = "#e31a1c") +
  geom_text(aes(label = signif, x = -0.15), size = 7, color = "#3c4d76") +
  geom_text(aes(label = path, x = -0.15), size = 6, color = "#3c4d76") +
  geom_vline(xintercept = 1, linetype = 2, linewidth = 0.5) +
  geom_vline(xintercept = max(snp_psgenes_sub$wmis_cv), linetype = 1, linewidth = 0.5) +
...
# Remove nickel due to low counts
snp_nsgenes <- snp_nsgenes %>% dplyr::filter(pathway != "Nickel")
...
snp_plot_nsgenes <- snp_nsgenes %>%
  ggviolin(x = "pathway", y = "wmis_cv", color = "pathway", add = "jitter", shape = "pathway",
           order = snp_sort_ns$pathway, palette = snp_sort_ns$color, legend = "none",
           size = 0.55, add.params = list(size = 3, alpha = 0.7)) +
  ...
  labs(x = "Pathway", y = "dN/dS") +
  stat_compare_means(method = "anova", label.y = 1.475, size = 6) +
  stat_compare_means(method = "t.test", label = "p.format", vjust = 0.25,
                     comparisons = list(c("Sulfur", "Copper"), c("Sulfur", "Iron"), c("Copper", "Iron")),
                     label.y = c(1.2, 1.27, 1.36), bracket.size = 0.25, size = 5) +
  theme(axis.text = element_text(size = 16),
        axis.title = element_text(size = 18))
ggsave("article_profiling/5-SNPs/dndscv/results/plot/plot_dnds_nsgenes.pdf", snp_plot_nsgenes, height = 9, width = 6)

# Merge NS genes to unique selected gene products
snp_nsgenes_sub <- snp_nsgenes %>%
  dplyr::group_by(product, pathway) %>%
  dplyr::summarise(dnds = mean(wmis_cv), .groups = "drop")
snp_nsgenes_sub_col <- c("#8c2421", "#666666", "#ddad3b")

# Dotplot of grouped products of NS genes
snp_plot_nsgensub <- snp_nsgenes_sub %>%
  dplyr::rename(Pathway = pathway) %>% #rename col.
  ggdotchart(x = "product", y = "dnds", color = "Pathway", y.text.col = TRUE, rotate = TRUE,
             palette = snp_nsgenes_sub_col, dot.size = 3, sorting = "descending") + #add = "segments"
  scale_y_continuous(limits = c(0, 1)) +
  geom_hline(yintercept = mean(snp_nsgenes$wmis_cv), linetype = 2, linewidth = 0.4) +
  labs(x = "NS gene products", y = "Mean dN/dS") +
  font("legend.title", size = 15) +
  theme(legend.text = element_text(size = 14),
        axis.text = element_text(size = 15),
        axis.title = element_text(size = 18))
ggsave("article_profiling/5-SNPs/dndscv/results/plot/plot_dnds_nsgenesprod.pdf", snp_plot_nsgensub, height = 10, width = 11)

# Global gene estimation of dN/dS (genes under PS and NS)
snp_estim <- snp_selcv %>%
  dplyr::select(gene_name, wmis_cv, pmis_cv) %>%
  dplyr::filter(wmis_cv != 0)

# Populate global dataset (runtime: ~2 min)
snp_gene2contig <- vector()
snp_contig2bin <- vector()
snp_genenames <- vector()
...
#add data for missing MAG: S15.19
snp_estim <- snp_estim %>%
  dplyr::mutate(phylum = case_when(is.na(phylum) ~ "Actinobacteriota", TRUE ~ phylum),
                completeness = case_when(is.na(completeness) ~ 98.29, TRUE ~ completeness),
                contamination = case_when(is.na(contamination) ~ 1.71, TRUE ~ contamination),
                predicted_genes = case_when(is.na(predicted_genes) ~ 2003, TRUE ~ predicted_genes))

# Any MAG >1? Yes: 2 (raw ratios)
snp_estim_glob <- snp_estim %>% dplyr::group_by(bin) %>% summarise(dnds_glob = mean(wmis_cv))

# Normalize dN/dS per gene by MAG contamination, then group by phylum
snp_estim_phy.filt <- snp_estim %>%
...
snp_estim_sort_phy <- snp_estim_phy.filt %>%
  dplyr::group_by(phylum) %>%
...
#snp_plot_estim_phy <- snp_estim_phy %>%
#  ggboxplot(x = "phylum", y = "dnds_glob", color = "phylum", add = "jitter",
#            order = snp_estim_sort_phy$phylum, palette = snp_estim_sort_phy$color,
#             size = 0.55, legend = "none", add.params = list(size = 3.25, alpha = 0.7)) +
#  geom_hline(yintercept = mean(snp_estim_phy$dnds_glob), linetype = 2, linewidth = 0.4) +
#  geom_hline(yintercept = 1, linetype = 1, linewidth = 0.4) +
#  labs(x = "Phylum", y = "Global normalized dN/dS ratio") +
#  stat_compare_means(method = "anova", label.y = 1.475, size = 6) +
#  stat_compare_means(method = "t.test", label = "p.format", vjust = 0.25,
#                     comparisons = list(c("Proteobacteria", "Actinobacteriota"),
#                                        c("Proteobacteria", "Acidobacteriota"),
#                                        c("Actinobacteriota", "Acidobacteriota")),
#                     label.y = c(1.25, 1.325, 1.4), bracket.size = 0.25, size = 5) +
#  theme(axis.text = element_text(size = 15),
#        axis.title = element_text(size = 18))
#ggsave("article_profiling/5-SNPs/dndscv/results/plot/plot_dnds_estim_phy.pdf", snp_plot_estim_phy, height = 9, width = 6)
##filtered
snp_plot_estim_phy <- snp_estim_phy.filt %>%
  ggboxplot(x = "phylum", y = "dnds_glob", color = "phylum", add = "jitter",
            order = snp_estim_sort_phy$phylum, palette = snp_estim_sort_phy$color,
             size = 0.55, legend = "none", add.params = list(size = 3.25, alpha = 0.7)) +
  geom_hline(yintercept = mean(snp_estim_phy.filt$dnds_glob), linetype = 2, linewidth = 0.4) +
  geom_hline(yintercept = 1, linetype = 1, linewidth = 0.4) +
  labs(x = "Phylum", y = "Global normalized dN/dS ratio") +
  stat_compare_means(method = "anova", label.y = 1.9, size = 6) +
  stat_compare_means(method = "t.test", label = "p.format", vjust = 0.25, tip.length = 0.02,
                     comparisons = list(c("Proteobacteria", "Actinobacteriota"),
                                        c("Proteobacteria", "Acidobacteriota"),
                                        c("Actinobacteriota", "Acidobacteriota")),
                     label.y = c(1.65, 1.725, 1.8), bracket.size = 0.25, size = 5) +
  theme(axis.text = element_text(size = 15),
        axis.title = element_text(size = 18))
ggsave("article_profiling/5-SNPs/dndscv/results/plot/filtered/plot_dnds_estim_phy.pdf", snp_plot_estim_phy, height = 9, width = 6)

# Aggregate normalized dN/dS ratios by bin
snp_estim_bin.filt <- snp_estim %>%
  dplyr::mutate(dnds_adj = wmis_cv * (1 - contamination / 100)) %>%
  dplyr::group_by(phylum, bin) %>%
  ...
  dplyr::mutate(bin = str_replace_all(bin, "SPAdes-MaxBin2Refined-", "")) %>% #trim bin names
  dplyr::mutate(bin = str_replace_all(bin, "SPAdes-MetaBAT2Refined-", "")) %>%
  dplyr::filter() #remove bin with

# Sort bins
snp_estim_sort_bin <- snp_estim_bin.filt %>% dplyr::arrange(dnds_glob)

# Barplot global dN/dS ratios by MAG bins
snp_plot_estim_bin <- snp_estim_bin.filt %>%
  dplyr::rename(Phylum = phylum) %>% #rename col.
  ggbarplot(x = "bin", y = "dnds_glob", color = "Phylum", fill = "Phylum", legend = "right",
            order = snp_estim_sort_bin$bin, palette = tree_colors, rotate = TRUE) +
  geom_hline(yintercept = 1, linetype = 1, linewidth = 0.4) +
  labs(x = "MAG", y = "Global normalized dN/dS ratio") +
  font("legend.title", size = 15) +
  theme(legend.text = element_text(size = 14),
        axis.text = element_text(size = 15),
        axis.title = element_text(size = 18))
ggsave("article_profiling/5-SNPs/dndscv/results/plot/filtered/plot_dnds_estim_bin.pdf", snp_plot_estim_bin, height = 9, width = 7)
...
## Figures 4-5 & supplementary 3

```{r fig4}
# Merge plots using patchwork
fig_4 <- wrap_plots(snp_plot_estim_bin, snp_plot_estim_phy, widths = c(0.6, 1)) +
  plot_annotation(tag_levels = list(c("a", "b"))) &
  theme(plot.tag = element_text(size = 20, face = "bold"))
ggsave("article_profiling/fig_4.pdf", fig_4, height = 10, width = 13)
ggsave("article_profiling/fig_4.png", fig_4, height = 10, width = 13)

fig_5a <- wrap_plots(snp_plot_nsgenes, snp_plot_nsgensub, widths = c(1, 1)) +
  plot_annotation(tag_levels = list(c("a", "b"))) &
  theme(plot.tag = element_text(size = 20, face = "bold"))
ggsave("article_profiling/fig_5a.pdf", fig_5a, height = 10, width = 15)
#ggsave("article_profiling/fig_5a.png", fig_5a, height = 10, width = 15)
fig_5b <- wrap_plots(snp_plot_psgenes, snp_plot_kos, design = "AABBB", heights = c(0.9, 1)) +
  plot_annotation(tag_levels = list(c("c", "", "d"))) &
  theme(plot.tag = element_text(size = 22, face = "bold"))
ggsave("article_profiling/fig_5b.pdf", fig_5b, height = 12, width = 22)
#ggsave("article_profiling/fig_5b.png", fig_5b, height = 12, width = 22)
#compose
fig_5 <- wrap_plots(wrap_plots(fig_5a, plot_spacer(), widths = c(1, 0.35)) / fig_5b) +
  plot_annotation(tag_levels = list(c("a", "b", "c", "", "d"))) &
  theme(plot.tag = element_text(size = 22, face = "bold"))
ggsave("article_profiling/fig_5.pdf", fig_5, height = 22, width = 22)
ggsave("article_profiling/fig_5.png", fig_5, height = 22, width = 22)
rm(fig_5a, fig_5b) #clean memory
