...
knitr::opts_knit$set(root.dir = '/Users/moises/Downloads/mags-all/')
...
#ggsave("fig_2.png", fig_2, height = 14, width = 18)
```

## Section 3: SNP Calling

### dN/dS analysis

```{r snps}
# Explore CDS annotations (from Bakta)
snp_refcds <- read.csv("5-SNPs/dndscv/buildref/cds_table.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

# Buildref testing for bacteria.. (1 bin)
#https://github.com/im3sanger/dndscv/issues/62
#buildref(cdsfile = "5-SNPs/dndscv/buildref/cds_node_25854.txt", genomefile = "5-SNPs/dndscv/buildref/node_25854.fa", outfile = "5-SNPs/dndscv/buildref/testref.rda")
#load("5-SNPs/dndscv/buildref/testref.rda")

# Buildref for all bins (very slow: ~2 hrs in building object)
#buildref(cdsfile = "5-SNPs/dndscv/buildref/cds_table.txt", genomefile = "5-SNPs/dndscv/buildref/S15_bins.fa", outfile = "5-SNPs/dndscv/buildref/S15_refcds.rda")

# Inspect refdb
load("5-SNPs/dndscv/buildref/S15_refcds.rda")
print(RefCDS[[1]])
rm(gr_genes, RefCDS)

# Get annotations from the VCF generated with SnpEff
#sampleID chr pos ref mut
snp_mutations <- read.csv("5-SNPs/dndscv/run/S15_mutations.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
rm(snp_mutations)

# Run driver discovery or genes under positive selection (PS) (runtime ~5 min)
#snp_dndsout <- dndscv(snp_mutations, refdb = "5-SNPs/dndscv/buildref/S15_refcds.rda", cv = NULL, max_muts_per_gene_per_sample = Inf, max_coding_muts_per_sample = Inf)
#save(snp_dndsout, file = "5-SNPs/dndscv/run/S15_dndsout.rda")
#load("5-SNPs/dndscv/run/S15_dndsout.rda")
...
mean(snp_selcv$wmis_cv) #mis avg.
...
#consider significant p-value -> pmis_cv < 0.05?
#snp_selcv %>% dplyr::filter(pmis_cv < 0.05) %>% nrow() #n=109523
snp_global_pos <- snp_selcv %>%
  dplyr::filter(wmis_cv > 1) %>%
  dplyr::summarise(genes = n())
snp_global_pos$label = "dN/dS > 1"
snp_global_neg <- snp_selcv %>%
  dplyr::filter(wmis_cv < 1 & wmis_cv != 0) %>%
  dplyr::summarise(genes = n())
snp_global_neg$label = "dN/dS < 1"
snp_global_neu <- snp_selcv %>%
  dplyr::filter(wmis_cv == 0) %>%
  dplyr::summarise(genes = n())
snp_global_neu$label = "dN/dS = 0"
snp_global_dnds <- rbind(snp_global_pos, snp_global_neg, snp_global_neu)
snp_global_dnds <- snp_global_dnds %>%
  dplyr::mutate(perc = round(genes * 100 / sum(snp_global_dnds$genes), 1)) %>% #add %
  dplyr::mutate(perc2 = paste0(genes, " (", perc, "%)")) #label to plot
rm(snp_global_pos, snp_global_neg, snp_global_neu) #clean memory

# Filter genes under PS:
#https://github.com/im3sanger/dndscv/issues/56
#wmis_cv: dN/dS for missense mutations
#pmis_cv: p-value < 0.05 is expected to occur by chance under neutrality (null hypothesis)
#p-values for multiple testing (q-values)
#qallsubs_cv = 144 genes; significant q-values (p-values adjusted for multiple testing)
snp_psgenes <- snp_selcv %>%
  dplyr::filter(wmis_cv > 1 & pmis_cv < 0.05) %>%
  dplyr::mutate(dndsr = n_mis / n_syn)
#snp_psgenes %>% dplyr::filter(dndsr == Inf) %>% nrow() #n=118

# Load contig to bin matrix
snp_bins <- read.csv("5-SNPs/dndscv/results/S15_DASTool_contig2bin.tsv", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

# Load gene name annotations (from genes.gff file)
snp_genids <- read.csv("5-SNPs/dndscv/results/gene_id_name.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
...
# Function that returns a row with taxa given a bin
fx_dndsTaxaBin <- function(b) {
  t <- mag_samples %>%
    dplyr::mutate(bin = paste0(mag_name, ".", mag_id)) %>%
    dplyr::filter(bin == b) %>%
    dplyr::select(phylum, genus, species, completeness, contamination)
  return(t)
}
...
fx_dndsNsPathway <- function(p) {
  ...
  if (grepl("sulfur", p, ignore.case = TRUE) == TRUE)
    return("Sulfur")
  if (grepl("mercury", p, ignore.case = TRUE) == TRUE)
    return("Mercury")
...
# Select PS and add taxonomic classification
snp_dnds <- snp_psgenes %>%
  #dplyr::filter(dndsr > 1) %>%
  dplyr::group_by(bin) %>%
...
# Subset to plot mutations -> synonymous/missense (non-syn.)/nonsense
snp_psgenes_sub <- snp_psgenes %>%
  dplyr::filter(wmis_cv > 5.5) %>%
  pivot_longer(cols = c(n_syn, n_mis, n_non), names_to = "mut", values_to = "n_mut")

# Barplot global dN/dS ratios
snp_plot_dndstot <- ggplot(snp_global_dnds, aes(x = genes, y = reorder(label, genes), fill = label)) +
  geom_col(position = "dodge2", fill = "#4c6194") +
  geom_text(aes(label = perc2), vjust = 0.5, hjust = -0.1, size = 2) +
  #scale_fill_manual(values = c("#c56c65", "#778ab3", "#529548")) +
  scale_x_continuous(limits = c(0, 125000)) +
  labs(x = "Number of genes", y = "") +
  theme_classic() +
  theme(legend.position = "none")
ggsave("5-SNPs/dndscv/results/plot/plot_dnds_global.pdf", snp_plot_dndstot, height = 3, width = 5)

# Plot global driver genes per taxa
snp_plot_psbins <- ggplot(snp_dnds, aes(x = num, y = reorder(bin, num), fill = bin)) +
  ...
  facet_grid(.~species)
ggsave("5-SNPs/dndscv/results/plot/plot_dnds_psbins.pdf", snp_plot_psbins, height = 8.5, width = 9.5)

# Lollipop plot of PS
#ggdotchart(snp_psgenes_sub, x = "name", y = "wmis_cv", color = "#E31A1C", rotate = TRUE, sorting = "descending", add = "segments", ggtheme = theme_pubr())
snp_plot_psgenes <- ggplot(snp_psgenes_sub,
  ...
  geom_vline(xintercept = max(snp_psgenes$wmis_cv), linetype = 1, linewidth = 0.1) +
  labs(x = "PS strength", y = "Gene CDS") +
  theme_classic()
ggsave("5-SNPs/dndscv/results/plot/plot_dnds_psgenes.pdf", snp_plot_psgenes, height = 9, width = 7)

# Plot mutations/genes per type
snp_plot_muts <- ggplot(snp_psgenes_sub, aes(x = n_mut, y = reorder(name, wmis_cv), fill = mut)) +
  ...
  theme(legend.position = c(0.85, 0.15),
        ...
        axis.line.y = element_blank(),
        axis.ticks.y = element_blank())
#ggsave("5-SNPs/dndscv/results/plot/plot_dnds_psmuts.pdf", snp_plot_muts, height = 9, width = 7)

snp_plot_psgenes <- (snp_plot_psgenes | snp_plot_muts) +
  plot_layout(nrow = 1, ncol = 2, widths = c(0.55, 0.5))
ggsave("5-SNPs/dndscv/results/plot/plot_dnds_psgenmut.pdf", snp_plot_psgenes, height = 9, width = 9)

# Negative selected (NS) genes with dN/dS < 1 (selected metals)
snp_nsgenes <- read.csv("5-SNPs/dndscv/results/selected_cds_metals.txt", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

# Parse product to pathway and add additional attributes (wmis_cv, pmis_cv)
#add dndsr, contig, bin**?
...
for (i in 1:nrow(snp_nsgenes)) {
  #work using dataframes, not lists
...
# Remove NA cells and significance: ie. BAOFFNEK_01342 not found
snp_nsgenes <- snp_nsgenes %>%
  dplyr::filter(!is.na(pathway) & !(is.na(wmis_cv))) %>%
  dplyr::filter(wmis_cv != 0 & pmis_cv < 0.05)

# Sort pathways and set colors
snp_sort_ns <- snp_nsgenes %>%
  dplyr::group_by(pathway) %>%
  dplyr::summarise(n = median(wmis_cv)) %>%
  dplyr::arrange(desc(n)) %>%
  as.data.frame()
snp_sort_ns$color <- c("#8c2421", "#4f4fad", "#ca6627", "#6a9b85")

# Violin plot for NS genes with global mutations
snp_plot_nsmut <- ggviolin(snp_nsgenes, x = "pathway", y = "wmis_cv", color = "pathway",
...
ggsave("5-SNPs/dndscv/results/plot/plot_dnds_nsmut.pdf", snp_plot_nsmut, height = 9, width = 7)

# Merge PS and NS gene names
snp_gmerge <- bind_rows(snp_psgenes, snp_nsgenes)
snp_gmerge <- snp_gmerge %>%
  dplyr::mutate(pathway = ifelse(is.na(pathway), "PS", pathway))
#snp_color_gm <- c("#ddad3b", "#4f4fad", "#6a9b85", "#ca6627", "#8c2421", "black")
snp_color_gm <- c("#8c2421", "#4f4fad", "#6a9b85", "#ca6627")

# Lollipop plot dN/dS x-axis range 0-2, y-axis PS genes vs selected CDS
snp_plot_gmerge <- ggdotchart(snp_nsgenes, x = "gene_name", y = "wmis_cv", color = "pathway",
                              xlab = "Gene CDS", ylab = "dN/dS", y.text.col = FALSE, rotate = TRUE,
                              palette = snp_color_gm, sorting = "descending", ggtheme = theme_pubr()) +
  scale_y_continuous(limits = c(0, 0.4)) +
  geom_hline(yintercept = mean(snp_nsgenes$wmis_cv), linetype = 2, linewidth = 0.2) +
  theme(axis.text.y = element_blank(),
        axis.ticks.y = element_blank())
ggsave("5-SNPs/dndscv/results/plot/plot_dnds_nspath.pdf", snp_plot_gmerge, height = 9, width = 7)

# Genes for enrichment (we need KEGG KOs..)
#snp_genenrich <- snp_psgenes$dndsr
#names(snp_genenrich) <- snp_psgenes$name
#snp_kegg <- enrichKEGG(snp_genenrich, "hsa", pvalueCutoff = 0.05)
...
```{r fig3}
# Merge plots using patchwork
fig_3 <- { { anova_boxplot_cu / anova_boxplot_fe +
    plot_layout(ncol = 1, widths = c(0.75)) } | anova_boxplot_s / kegg_bubbles_plot } +
  plot_annotation(tag_levels = list(c("a", "b", "c", "d"))) &
  theme(plot.tag =  element_text(face = "bold"))
ggsave("fig_3.pdf", fig_3, height = 14, width = 18)
ggsave("fig_3.png", fig_3, height = 14, width = 18)
```
